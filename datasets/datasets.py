...
class ImageDataset(Dataset):
    def __init__(self, root, unaligned=False, mode='train', skip=False):
        ...
        # self.files_orimask = sorted(glob.glob(os.path.join('/media/disk/zys/ISTD_Dataset', '%s/train_B' % mode) + '/*.*'))
    ...
    def __getitem__(self, index):
        ...
        # if True:
        #     tmporimask = io.imread(self.files_orimask[index % len(self.files_orimask)])
        #     item_F = item_F - (item_F & tmporimask)
        ...
        # orimask = io.imread(self.files_orimask[index % len(self.files_orimask)])
        # orimask = resize(orimask, (448, 448))

        # orimask[orimask > 0.5] = 1
        # orimask[orimask <= 0.5] = 0
        #
        # orimask = orimask[i:i + 400, j:j + 400]
        # if k>50:
        #     orimask=np.fliplr(orimask)
        # orimask=torch.from_numpy(orimask.copy()).float()
        # orimask=orimask.view(400,400,1)
        # orimask=orimask.transpose(0, 1).transpose(0, 2).contiguous()

        is_skip = False
        if self.skip and ((torch.sum(item_C) / torch.sum(orimask)) < 0.1):
            is_skip = True

        return {'A': item_A, 'B': item_B, 'C': item_C, 'D': item_D,
                'E': item_E, 'F': item_F, 'name': self.files_A[index % len(self.files_A)], 'is_skip': is_skip}
