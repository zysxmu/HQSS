...
maskdir = '~/Documents/shadow-results/SRD/test/test_B/';
MD = dir([maskdir '/*.jpg']);

%maskdir = 'D:\shadow-removal\ISTD\test\test_B\';
%MD = dir([maskdir '/*.png']);



% result directory|结果路径
% shadowdir = 'C:\资料\2020-09-G2R\G2R-ShadowNet\ckptfs\B_100_mask6\';  
shadowdir = '~/Desktop/best_PI/';  % bs1_B_100_mask6
SD = dir([shadowdir '/*.jpg']);

%shadowdir = 'C:\Users\zys\Desktop\best\'; 
%SD = dir([shadowdir '/*.png']);



% ground truth directory|GT路径
freedir = '~/Documents/shadow-results/SRD/test/test_C/'; %
FD = dir([freedir '/*.jpg']);

%freedir = 'D:\shadow-removal\ISTD_adjusted\test_C_fixed_official\'; %AISTD
%FD = dir([freedir '/*.png']);
...
% for i=1:1
%     sname = strcat(shadowdir,name); 
%     fname = strcat(freedir,name); 
%     mname = strcat(maskdir,name); 
    s=imread(sname);
    f=imread(fname);
    m=imread(mname);
